    ...
    while i < len(expr):
        ...
        if expr[i].isalpha():
            ...
            while i < len(expr) and expr[i].isalnum():
                ...
                i += 1
            output.append(
                vars.get(var_name, '0' * len(next(iter(vars.values())))))  # 使用默认长度
            continue
...
def calculate_postfix(postfix_expr, vars):
    ...
    for token in postfix_expr:
        if token.isdigit() or (token[0] == '!' and token[1:].isdigit()):
            ...
            if token[0] == '!':
                num = bin(~int(token[1:], 2) & (
                    2 ** len(token[1:]) - 1))[2:].zfill(len(token[1:]))
                stack.append(num)
            else:
            ...
            if token == '!':
                operand = stack.pop()
                result = bin(~int(operand, 2) & (
                    2 ** len(operand) - 1))[2:].zfill(len(operand))
            else:
                right = stack.pop()
                left = stack.pop()
                if token == '+':
